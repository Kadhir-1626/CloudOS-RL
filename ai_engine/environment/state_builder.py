...
_BASE_LATENCY_MS: Dict[str, float] = {
    "us-east-1": 45.0,  "us-west-2": 72.0,
    "eu-west-1": 110.0, "eu-central-1": 115.0,
    "ap-southeast-1": 185.0, "ap-northeast-1": 168.0,
    "us-central1": 55.0, "europe-west4": 108.0,
    "eastus": 48.0, "westeurope": 112.0,
}
...
class StateBuilder:
    ...
    def build(
        ...
        for i, region in enumerate(REGIONS):
            state[10 + i] = (
                pricing.get(region, {}).get("on_demand_per_vcpu_hr", 0.048)
                / self._p_norm
            )

        # ── carbon (10)
        for i, region in enumerate(REGIONS):
            state[20 + i] = carbon.get(region, 400.0) / self._c_norm
