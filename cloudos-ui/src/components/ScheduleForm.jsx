    ...
    <div className="card">
      ...
      <div style={row}>
        <div style={col}>
          ...
          <select value={form.workload_type} onChange={e => set('workload_type', e.target.value)}>
      ...
      <div style={row}>
        <div style={col}>
          ...
          <input type="number" min={0.25} step={0.25} value={form.cpu_request_vcpu}
            onChange={e => set('cpu_request_vcpu', e.target.value)} />
        </div>
        <div style={col}>
          <label>Memory (GB)</label>
          <input type="number" min={0.5} step={0.5} value={form.memory_request_gb}
            onChange={e => set('memory_request_gb', e.target.value)} />
        </div>
        <div style={col}>
          <label>GPU Count</label>
          <input type="number" min={0} max={16} value={form.gpu_count}
      ...
      <div style={row}>
        ...
        <div style={col}>
          ...
          <input type="number" min={0.1} step={0.1} value={form.expected_duration_hours}
            onChange={e => set('expected_duration_hours', e.target.value)} />
        </div>
        <div style={col}>
          <label>SLA Latency (ms)</label>
          <input type="number" min={10} value={form.sla_latency_ms}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
        <button
          ...
          style={{
            ...
            border: `1px solid ${form.is_spot_tolerant ? 'var(--green)' : 'var(--border)'}`,
        ...
        <span style={{ color: 'var(--muted)', fontSize: 12 }}>
          {form.is_spot_tolerant ? 'Eligible for spot pricing (up to 70% savings)' : 'On-demand only'}
        </span>
      </div>

      {error && (
        <div style={{
          background: '#ef444415', border: '1px solid var(--red)',
      ...
      >
        {loading ? <><Loader size={15} style={{ animation: 'spin 1s linear infinite' }} /> Scheduling…</> : <><Send size={15} /> Schedule Workload</>}
      </button>
